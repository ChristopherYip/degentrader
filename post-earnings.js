...
// Data: Finnhub (calendar, quotes, profile, metrics, news), FMP (chart history),
//       QuickChart (chart image). Logo + CEO images come from vetted repo assets:
//         assets/logos/{SYMBOL}.png|jpg|jpeg|webp
//         assets/ceos/{SYMBOL}.png|jpg|jpeg|webp
//       Missing asset = image skipped, post still goes out. Dedupe via Postgres.
...
async function finnhub(pathName, params = {}) {
  ...
  const res = await fetch(`https://finnhub.io/api/v1/${pathName}?${qs}`);
  if (!res.ok) {
...
// Extra metrics for the post body. Everything here is optional.
...
// Load a vetted image from the repo's assets folder, e.g. assets/logos/NVDA.png.
// Tries png/jpg/jpeg/webp. Returns { buffer, mimeType } or null if not found.
function loadLocalImage(subdir, symbol) {
  for (const ext of IMAGE_EXTENSIONS) {
    const filePath = path.join(ASSETS_DIR, subdir, `${symbol}.${ext}`);
    try {
      if (!fs.existsSync(filePath)) continue;
      const buf = fs.readFileSync(filePath);
      if (buf.length > MAX_IMAGE_BYTES) {
        console.log(`${symbol}: ${subdir} asset too large (${(buf.length / 1e6).toFixed(1)}MB), skipping`);
        return null;
      }
      const mimeType = detectImageType(buf);
      if (!mimeType) {
        console.log(`${symbol}: ${subdir} asset has unrecognized image type, skipping`);
        return null;
      }
      return { buffer: buf, mimeType };
    } catch (err) {
      console.log(`${symbol}: failed reading ${subdir} asset (${err.message})`);
      return null;
    }
  }
  return null;
...
// 3-month price chart in a plain "source" style (Google Finance look):
// white background, red/green line, faint grid, price axis on the right.
// Price history from FMP (Finnhub candles are paid).
async function renderChartImage(symbol, companyName) {
  ...
  const up = last >= first;
  const lineColor = up ? '#137333' : '#d93025';
  const periodPct = ((last - first) / first) * 100;
  const arrow = up ? '▲' : '▼';
  const lastStr = last >= 1000
  ...
  const config = {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: up ? 'rgba(19,115,51,0.06)' : 'rgba(217,48,37,0.06)',
        fill: true,
        pointRadius: 0,
        borderWidth: 1.5,
        tension: 0,
    ...
    options: {
      layout: { padding: { top: 4, left: 8, right: 4, bottom: 4 } },
      plugins: {
        legend: { display: false },
        title: {
          display: true,
          align: 'start',
          text: `${symbol}  $${lastStr}  ${arrow} ${Math.abs(periodPct).toFixed(1)}% (3M)`,
          color: lineColor,
        ...
        subtitle: {
          display: true,
          align: 'start',
          text: `${companyName || symbol} · daily close`,
          color: '#5f6368',
          font: { size: 12 },
          padding: { bottom: 10 },
        },
      ...
      scales: {
        x: {
          ticks: { color: '#80868b', maxTicksLimit: 6, font: { size: 11 } },
          grid: { display: false },
          border: { color: '#dadce0' },
        },
        y: {
...
// Gather up to 2 images: a vetted CEO photo or logo (alternating per post,
// with fallback to the other if missing), then the chart. Every step is non-fatal.
async function gatherImages(symbol, companyName, preferCeo) {
  const images = [];

  const primaryDir = preferCeo ? 'ceos' : 'logos';
  const backupDir = preferCeo ? 'logos' : 'ceos';
  let asset = loadLocalImage(primaryDir, symbol);
  let used = primaryDir;
  if (!asset) {
    asset = loadLocalImage(backupDir, symbol);
    used = backupDir;
  }
  if (asset) {
    images.push(asset);
    console.log(`${symbol}: ${used} asset ready (lead image)`);
  } else {
    console.log(`${symbol}: no CEO/logo assets, chart only`);
  }

  try {
    images.push(await renderChartImage(symbol, companyName));
...
async function runPostingWindow(db, isMorning, todayET) {
  ...
  // Alternate the lead image (CEO vs logo) across all posts, using the
  // running total in posted_earnings — even count = CEO, odd = logo.
  const countRes = await db.query('SELECT COUNT(*)::int AS n FROM posted_earnings');
  const totalPosted = countRes.rows[0]?.n || 0;
  ...
  for (const e of candidates) {
    ...
    // Images: alternating vetted CEO/logo lead + generated chart — all non-fatal
    const preferCeo = (totalPosted + posted) % 2 === 0;
    const images = await gatherImages(e.symbol, extras.name, preferCeo);
    const mediaIds = [];
