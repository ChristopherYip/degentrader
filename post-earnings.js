...
// Runs every 15 min via Railway cron. Three// post-earnings.js — Long-form earnings reaction posts for @DegenTrader
// Requires X Premium (posts exceed 280 chars) + images attached.
...
// Gather up to 3 images: generated chart + vetted logo + vetted CEO photo.
// Every step is non-fatal.
async function gatherImages(symbol, companyName) {
  const images = [];
  try {
  ...
  const logo = loadLocalImage('logos', symbol);
  if (logo) {
    images.push(logo);
    console.log(`${symbol}: logo asset ready`);
  } else {
    console.log(`${symbol}: no logo asset, skipping`);
  }
  const ceo = loadLocalImage('ceos', symbol);
  if (ceo) {
    images.push(ceo);
    console.log(`${symbol}: CEO asset ready`);
  } else {
    console.log(`${symbol}: no CEO asset, skipping`);
  }
  return images;
}
...
async function runPostingWindow(db, isMorning, todayET) {
  ...
  for (const e of candidates) {
    ...
    // Images: generated chart + vetted repo assets — all non-fatal
    const images = await gatherImages(e.symbol, extras.name);
    const mediaIds = [];
